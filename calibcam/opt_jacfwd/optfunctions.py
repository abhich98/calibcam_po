...
def get_precalc(model: str):
    num_vars = 7 if model == "omnidir" else 6

    return {
        'objfunc': jit(opt_ag.obj_fcn(model)),
        'jacobians': [jit(jacobian(opt_ag.obj_fcn(model), i_var)) for i_var in range(num_vars)]
    }


def obj_fcn_wrapper(model: str):
    if model == "omnidir":
        return obj_fcn_omnidir_wrapper
    else:
        return obj_fcn_pinhole_wrapper


def obj_fcn_pinhole_wrapper(vars_opt, args):
    ...
    rvecs_cams, tvecs_cams, cam_matrices, ks, rvecs_boards, tvecs_boards = \
        optimization.unravel_pinhole_vars_full(vars_full, n_cams)
...
def obj_fcn_omnidir_wrapper(vars_opt, args):
    corners = args['corners'].copy()  # copy is necessary since this is a reference, so further down, nans
    # will be replaced with 0 globally
    corners_mask = np.isnan(corners)
    corners[corners_mask] = 0
    ...
    rvecs_cams, tvecs_cams, cam_matrices, xis, ks, rvecs_boards, tvecs_boards = \
        optimization.unravel_omnidir_vars_full(vars_full, n_cams)
...
def obj_fcn_jacobian_wrapper(model: str):
    if model == "omnidir":
        return obj_fcn_omnidir_jacobian_wrapper_full
    else:
        return obj_fcn_pinhole_jacobian_wrapper_full


def obj_fcn_pinhole_jacobian_wrapper_full(vars_opt, args):
    ...
    rvecs_cams_mask, tvecs_cams_mask, cam_matrices_mask, ks_mask, rvecs_boards_mask, tvecs_boards_mask = \
        optimization.unravel_pinhole_vars_full(args['mask_opt'], n_cams)
...
def obj_fcn_omnidir_jacobian_wrapper_full(vars_opt, args):
    corners = args['corners']
    corners_mask = np.isnan(corners)
    board_coords_3d_0 = args['board_coords_3d_0']
    ...
    rvecs_cams_mask, tvecs_cams_mask, cam_matrices_mask, xis_mask, ks_mask, rvecs_boards_mask, tvecs_boards_mask = \
        optimization.unravel_omnidir_vars_full(args['mask_opt'], n_cams)

    var_masks = [rvecs_cams_mask, tvecs_cams_mask, cam_matrices_mask, xis_mask, ks_mask, rvecs_boards_mask, tvecs_boards_mask]
