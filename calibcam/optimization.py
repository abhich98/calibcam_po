...
def unravel_pinhole_vars_full(vars_full, n_cams):
...
def unravel_omnidir_vars_full(vars_full, n_cams):
...
def make_initialization(calibs, corners, board_params, opts):
    ...
    camera_params = make_cam_params(opts['model'], calibs, opts_free_vars)
    ...
    mask_free_input = make_free_parameter_mask(opts['model'], calibs, opts_free_vars, opts['coord_cam'])
    vars_free = vars_full[mask_free_input]

    return vars_free, vars_full, mask_free_input


def make_cam_params(model, calibs, opts_free_vars):
    ...
    xis = np.zeros(shape=(len(calibs), 1))  # Only for omnidir
    ...
    camera_params = np.concatenate((
        ...
        tvecs_cam.ravel(),
        cam_mats.ravel(),
        xis.ravel() if model == "omnidir" else [],
        ks.ravel(),
    ), axis=0)
...
def make_free_parameter_mask(model, calibs, opts_free_vars, coord_cam_idx):
    ...
    if model == "omnidir":
        ...
        xis_mask = np.asarray([], dtype=bool)
...
def unravel_to_calibs(vars_opt, model, args):
    # Fill vars_full from initialization with vars_opts
    vars_full, n_cams = make_vars_full(vars_opt, args, verbose=False)

    # Unravel inputs.
    if model == "omnidir":
        rvecs_cams, tvecs_cams, cam_matrices, xis, ks, rvecs_boards, tvecs_boards = unravel_omnidir_vars_full(vars_full, n_cams)
    else:
        rvecs_cams, tvecs_cams, cam_matrices, ks, rvecs_boards, tvecs_boards = unravel_pinhole_vars_full(vars_full, n_cams)
