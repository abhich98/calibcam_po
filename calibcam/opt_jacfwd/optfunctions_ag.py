...
def obj_fcn(model: str):
    if model == "omnidir":
        return obj_fcn_omnidir
    else:
        return obj_fcn_pinhole


def obj_fcn_pinhole(rvecs_cams, tvecs_cams, cam_matrices, ks, rvecs_boards, tvecs_boards, board_coords_3d_0, corners):
...
def obj_fcn_omnidir(rvecs_cams, tvecs_cams, cam_matrices, xis, ks, rvecs_boards, tvecs_boards, board_coords_3d_0, corners):
    rvecs_cams = rvecs_cams.reshape(-1, 3)
    tvecs_cams = tvecs_cams.reshape(-1, 3)
    cam_matrices = cam_matrices.reshape(-1, 3, 3)
    xis = xis.reshape(-1, 1)
